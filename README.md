...
```bash
docker pull eppicenter/sampledb:v2.7.1
```
...
```bash
docker build -t eppicenter/sampledb:v2.7.1 .
```
...
```bash
docker run -d -p 8080:3838 -v sampledb_database:/usr/local/share/sampleDB --restart unless-stopped --name sampleDB eppicenter/sampledb:v2.7.1
```
